...
const questionCreateSchema = z
  ...
  .refine(
    ...
    {
      message: 'SCQ must have exactly 1 correct answer, MCQ must have at least 1 correct answer',
    }
  )
...
export async function questionsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: z.infer<typeof questionCreateSchema> }>(
    ...
    async (request, reply) => {
      ...
      const { body, type, difficulty, explanation, categoryId, answers } = parsed.data
  ...
  fastify.put<{ Params: { id: string }; Body: z.infer<typeof questionUpdateSchema> }>(
    '/:id',
    { preHandler: [adminAuthMiddleware] },
    async (request, reply) => {
      const { id } = request.params
